...
let canvasWidth = 800;
let drawHeight = 400;
let controlHeight = 100; // Increased to accommodate FFT size control
...
let currentAmp = 0.5;
let currentFFTSize = 1024; // Default FFT size
...
function setup() {
  ...
  // Create FFT with moderate smoothing and default resolution
  fft = new p5.FFT(0.8, currentFFTSize);
...
function checkFFTSizeChange() {
  let selectedSize = parseInt(fftSizeRadio.value());
  if (selectedSize !== currentFFTSize) {
    currentFFTSize = selectedSize;
    // Recreate FFT with new size
    fft = new p5.FFT(0.8, currentFFTSize);
  }
}
...
// this is called only once in the setup()
function drawControlLabels() {
  ...
  text('FFT Size: ' + currentFFTSize + ' bins', 20, drawHeight + 85);
  ...
  fill('black');
  textAlign(RIGHT, CENTER);
  let freqResolution = (22050 / currentFFTSize).toFixed(1);
  text('Freq Resolution: ' + freqResolution + ' Hz/bin', canvasWidth - 20, drawHeight + 55);
}
...
function createControls() {
  ...
  // FFT Size radio buttons
  fftSizeRadio = createRadio();
  fftSizeRadio.option('512', 512);
  fftSizeRadio.option('1024', 1024);
  fftSizeRadio.option('2048', 2048);
  fftSizeRadio.option('4096', 4096);
  fftSizeRadio.option('8192', 8192);
  fftSizeRadio.selected('2048'); // Default selection to match your preference
  fftSizeRadio.position(sliderLeftMargin + 220, drawHeight + 65);
  fftSizeRadio.style('font-size', '14px');
  fftSizeRadio.style('font-family', 'Arial, Helvetica, sans-serif');
  
  // FFT Size label
  let fftLabel = createP('FFT Size:');
  fftLabel.position(sliderLeftMargin + 220, drawHeight + 45);
  fftLabel.style('margin', '0');
  fftLabel.style('padding', '0');
  fftLabel.style('font-size', '12px');
  fftLabel.style('font-family', 'Arial, Helvetica, sans-serif');
  fftLabel.style('font-weight', 'bold');
  fftLabel.style('color', 'black');
  // If this is in the draw it would be fewer lines..
  // text('FFT Size:', sliderLeftMargin + 220, drawHeight + 65)
}
